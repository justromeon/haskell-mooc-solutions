...
treeMax Empty        = 0
treeMax (Node n l r) = maximum [n, treeMax l, treeMax r]
...
isOrdered (Node x l r) = testWithRoot (<) l x && testWithRoot (>) r x && isOrdered l && isOrdered r
  where
    testWithRoot cond tree val = case valAtRoot tree of
      Nothing -> True
      Just y  -> cond y val
