...
greetText name = T.pack "Hello, " <> T.take 15 name <> T.pack (if T.length name > 15 then "...!" else "!")
...
shout :: T.Text -> T.Text
shout = T.intercalate (T.pack " ") . go . T.splitOn (T.pack " ")
  where
    go [] = []
    go (x:xs) = T.toUpper x : dont xs
...
longestRepeat = foldr max 0 . map (fromIntegral . TL.length) . TL.group . TL.fromStrict
...
takeStrict n lazyText = TL.toStrict (TL.take n lazyText)
...
byteRange byteStr
    | B.null byteStr = 0
    | otherwise      = B.maximum byteStr - B.minimum byteStr
...
pingpong byteStr = BL.cycle $ BL.fromStrict (byteStr <> B.reverse byteStr)
