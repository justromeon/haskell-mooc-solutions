...
inputInOutput input output = forAll (elements input) isElem
  where
    isElem x = elem x (map fst output)
...
outputInInput input output = forAll (elements output) isCorrectCount
  where
    isCorrectCount (x,n) = n == length (filter (==x) input) 
...
frequenciesProp freq (NonEmpty input)= foldr1 (.&&.)
  $ map (\prop -> prop input output) [sumIsLength, inputInOutput, outputInInput]
  where
    output = freq input
...
genList = do
  len <- chooseInt (3,5)
  xs <- vectorOf len (chooseInt (1,10))
  return (sort xs)
