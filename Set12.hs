...
count x = foldr (\y total -> if y == x then total+1 else total) 0
...
inBoth foldable = foldr (\x acc -> if elem x foldable then x:acc else acc) [] 
...
instance Foldable List where
  foldr _ v Empty        = v
  foldr f v (LNode x xs) = f x (foldr f v xs)
...
instance Foldable TwoList where
  foldr _ v TwoEmpty         = v
  foldr f v (TwoNode x y zs) = f x (f y (foldr f v zs))
...
sumTree :: Monoid m => Tree m -> m
sumTree Leaf                 = mempty
sumTree (Node x lTree rTree) = sumTree lTree <> x <> sumTree rTree
