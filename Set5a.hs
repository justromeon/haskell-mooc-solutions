...
buyOneMore (MkShoppingEntry item price quantity) = MkShoppingEntry item price (quantity+1) 
...
rgb Green = [0,1,0]
rgb Blue  = [0,0,1]
rgb (Invert color) = map (\n -> 1- n) (rgb color)
rgb (Mix c1 c2) = zipWith (\n m -> (n+m) / 2) (rgb c1) (rgb c2)
...
prettyPrint bin = go bin ""
  where
    go End     s = s
    go (O bin) s = go bin ('0':s)
    go (I bin) s = go bin ('1':s)

fromBin :: Bin -> Int
fromBin End = 0
fromBin (O bin) = 0 + (2 * fromBin bin)
fromBin (I bin) = 1 + (2 * fromBin bin)

toBin :: Int -> Bin
toBin 0 = O End
toBin 1 = I End
toBin n = convert remainder $ toBin quotient
  where
