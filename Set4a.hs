...
longest :: Ord a => [[a]] -> [a]
longest xs = minimumBy (comparing head) $ filter ((==maxLen) . length) xs
  where
...
transfer from to amount bank
    | Map.member from bank && Map.member to bank && sufficientBalance && amount > 0 =
      Map.adjust (+amount) to $ Map.adjust (subtract amount) from bank
    | otherwise = bank
  where
    sufficientBalance = maybe False (>=amount) (Map.lookup from bank)
