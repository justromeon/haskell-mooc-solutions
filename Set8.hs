...
combine combiner (Picture f) (Picture g) = Picture (\coord -> combiner (f coord) (g coord))
...
rectangle x0 y0 w h = Shape f
  where
    f (Coord x y) = x >= x0 && x <= (x0+w-1) && y >= y0 && y <= (y0+h-1)
------------------------------------------------------------------------------
...
flipX (Picture p) = Picture (\coord -> p $ flipCoordX coord)

flipY :: Picture -> Picture
flipY (Picture p) = Picture (\coord -> p $ flipCoordY coord)
...
instance Transform Blur where
  apply Blur (Picture p) = Picture (\coord -> blendColors $ map p $ joinNeighbors coord)
    where
      joinNeighbors coord = [coord, above coord, below coord, left coord, right coord] 
...
blendColors colorList = Color red green blue
  where
    red   = sum (map getRed colorList) `div` len
    green = sum (map getGreen colorList) `div` len
    blue  = sum (map getBlue colorList) `div` len
